...
function formatAtRules (root, params) {
  ...
  root.walkAtRules(function (atrule, index) {
    ...
    var atArrary = ['import', 'media', 'font-face']
    ...
    if (index === 0 && parentType === 'root') {
      ...
      if (parentType === 'atrule' || parentType === 'rule') {
        ...
        formatDecls.formatDecls(atrule, indentation, indentWidth, stylelint)
      }
    ...
    if(atArrary.indexOf(atrule.name) != -1) {
	    atrule.raws.between = ''
	  ...
	  if (stylelint && stylelint['string-quotes']) {
    	atrule.params = declarationStingQuotes(stylelint, atrule.params)
  	}
